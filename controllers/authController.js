...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  // Allow role assignment if provided (for admin creating guide accounts)
  if (req.body.role && ['guide', 'lead-guide', 'admin'].includes(req.body.role)) {
    userData.role = req.body.role;
  }

  const newUser = await User.create(userData);

  // Send welcome email in the background (only if not admin-created)
  if (!req.body.role) {
    sendWelcomeEmail(newUser, req);
  }

  // If role is being set, don't auto-login (return JSON instead)
  if (req.body.role) {
